...
void GLViewer::mouse_press_ogl(int32_t button, double x, double y )
{
	if (locked_)
		return;
	if (button == 0)
	{
		...
		need_redraw_ = true;
		double b = std::min(0.1 * width(), 0.1 * height());
		aroundZ = ((abs(x - width()/2) > width()/2 - b) || (abs(y - height()/2) > height()/2 - b));
	}
}
...
void GLViewer::mouse_move_ogl(double x, double y)
{
	...
	if ((mouse_buttons_ & 1) && ((std::abs(dx)+ std::abs(dy))>0.0))
	{
		...
		double n = -std::copysign(1.0, 2.0 * u.dot(axis1));
		Eigen::Vector3d axis = aroundZ ? Eigen::Vector3d(0, 0, n/abs(n)) : axis1;
